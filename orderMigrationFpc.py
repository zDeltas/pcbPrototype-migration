...
class Fr4Other:
    def __init__(self, id, numcommande, doyouneed, doyougerb, datecode, datecodewhere, rohslogo, whererohlogo, ullogo,
                 whereullogo, smallesttracewidth, stiffener_fr4, zif, finished_hole, stencil, thickness, padreduction):
        self.id = id
        ...
        self.stiffener_fr4 = self.treatmentStiffenerFr4(stiffener_fr4, numcommande)
        self.zif = self.treatmentZif(zif, numcommande)
        self.finished_hole = self.treatmentFinishedHole(finished_hole, numcommande)
    ...
    def treatmentStiffenerFr4(self, stiffener_fr4, numcommande):
        match stiffener_fr4:
            case "No":
                return "0"
            case "1":
            ...
            case "2":
                return "2"
            case _:
                raise ValueError("[treatmentStiffenerFr4] - " + str(numcommande) + " value : " + str(stiffener_fr4))

    def treatmentZif(self, stiffener_fr4, numcommande):
        match stiffener_fr4:
            case "No":
                return "0"
            case "1":
            ...
            case "2":
                return "2"
            case _:
                raise ValueError("[treatmentZif] - " + str(numcommande) + " value : " + str(stiffener_fr4))

    def treatmentFinishedHole(self, stiffener_fr4, numcommande):
        match stiffener_fr4:
            case "0.15":
                return "0.15"
            case "0.20":
            ...
            case "0.25":
                return "0.25"
            case _:
                raise ValueError("[treatmentFinishedHole] - " + str(numcommande) + " value : " + str(stiffener_fr4))
...
class BasicRequi:
    def __init__(self, id, numcommande, layer, pi_thickness, adhesive, surface_treatment, screen_printing,
                 color_screen, base_copper):
        self.id = id
        self.numcommande = numcommande
        self.layer = self.treatmentLayer(layer, numcommande)
        self.pi_thickness = self.treatmentPiThickness(pi_thickness, numcommande)
        self.adhesive = self.treatmentAdhesive(adhesive, numcommande)
    ...
    def treatmentPiThickness(self, pcb_thickness, numcommande):
        match pcb_thickness:
            case "12.5":
                return "12.5"
            case "25":
            ...
            case "50":
                return "50"
            case _:
                raise ValueError("[treatmentPiThickness] - " + str(numcommande) + " value : " + str(pcb_thickness))

    def treatmentAdhesive(self, pcb_thickness, numcommande):
        match pcb_thickness:
            case "12.5":
                return "12.5"
            case "20":
...
def getOrders():
    query_members = "SELECT typedelacommande, typecommande, refmembre, numcommande, partnumber, version, \"system\", " \
                    ...
                    "from commande where date > '2023-05-01' and typecommande = 'FPC'"

    # "from commande where date > '2020-01-01' and typecommande = 'FPC'"
...
def setOrders():
    ...
    insert_order_content = "INSERT INTO order_content (id, type_design, length, width, quantity_pcb_panel, cross_board, quantity_different_pcb_type," \
                           "is_design_by_customer, custom_panel_id, surface_treatment, " \
                           "screen_printing_position, screen_printing_color, layers, pl_thickness, adhesive_thickness," \
                           "stiffener, zif_connector, copper_thickness_base, trace,finished_hole_size, " \
                           "date_code, date_code_position, rohs_logo, rohs_logo_position, ul_logo," \
    ...
    select_fpc_basic_requi = "select id, numcommande, layer, pi_thickness, adhesive, surface_treatment,  " \
                             "screen_printing, color_screen, base_copper " \
                             "from fpc_basic_requi where numcommande = %s;"

    select_fpc_other = "select id, numcommande, doyouneed, doyougerb, datecode, datecodewhere, rohslogo, " \
                       "whererohlogo, ullogo, whereullogo, smallesttracewidth, " \
                       "stiffener_fr4, zif, finished_hole, stencil, thickness, padreduction " \
                       "from fpc_other where numcommande = %s;"

    select_fpc_dimension = "select id, numcommande, panel, unit, unit_length, unit_width, paneltype, typeallpcb, varioostype," \
                           "panel_lenght_allpcb, panel_width_allpcb, allpcbxout_allpcb, panel_allpcb," \
                           "panel_varioos_lenght, panel_varioos_width, differentpcb_type, allpcbxout_varioos," \
                           "panelpopup_unit_X, panelpopup_unit_Y, panelpopup_nbpcb_X, panelpopup_nbpcb_Y," \
                           "panelpopup_border_top, panelpopup_border_bottom, panelpopup_border_right," \
                           ...
                           "panel_popup_pcb " \
                           "from fpc_dimension where numcommande = %s;"
    ...
    for order in orders:
        ...
        try:
            ...
            cursorMSQL.execute(select_fpc_basic_requi, numcommand)
            select_fpc_basic_requi_result = cursorMSQL.fetchall()

            if (len(select_fpc_basic_requi_result) == 0):
                raise ValueError("BasicRequi missing")
            if (len(select_fpc_basic_requi_result) > 1):
                select_fpc_basic_requi_result = select_fpc_basic_requi_result[-1]
            if (len(select_fpc_basic_requi_result) == 1):
                select_fpc_basic_requi_result = select_fpc_basic_requi_result[0]
            basicRequi = BasicRequi(*select_fpc_basic_requi_result)

            cursorMSQL.execute(select_fpc_other, numcommand)
            select_fpc_other_requi_result = cursorMSQL.fetchall()

            if (len(select_fpc_other_requi_result) == 0):
                raise ValueError("Fr4Other missing")
            if (len(select_fpc_other_requi_result) > 1):
                select_fpc_other_requi_result = select_fpc_other_requi_result[-1]
            if (len(select_fpc_other_requi_result) == 1):
                select_fpc_other_requi_result = select_fpc_other_requi_result[0]
            fr4Other = Fr4Other(*select_fpc_other_requi_result)

            cursorMSQL.execute(select_fpc_dimension, numcommand)
            select_fpc_dimension_result = cursorMSQL.fetchall()

            if (len(select_fpc_dimension_result) == 0):
                raise ValueError("Fr4Dimension missing")
            if (len(select_fpc_dimension_result) > 1):
                select_fpc_dimension_result = select_fpc_dimension_result[-1]
            if (len(select_fpc_dimension_result) == 1):
                select_fpc_dimension_result = select_fpc_dimension_result[0]
            fr4Dimension = Fr4Dimension(*select_fpc_dimension_result)
            ...
            order_content_values_ = (
                ...
                isDesignByCustomer, uuid_custom_panel, basicRequi.surface_treatment,
                basicRequi.screen_printing, basicRequi.color_screen, basicRequi.layer,
                basicRequi.pi_thickness, basicRequi.adhesive, fr4Other.stiffener_fr4, fr4Other.zif,
                basicRequi.base_copper, fr4Other.smallesttracewidth,
                fr4Other.finished_hole, fr4Other.datecode,
